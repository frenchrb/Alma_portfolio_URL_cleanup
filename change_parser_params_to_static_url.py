...
def worker():
    while True:
        ...
        if get_response.status_code == 200:
            ...
            output.append((port['row'], existing_url_col_index, url))
            
            if 'ID=' in url:
                new_url = url.replace('ID', 'jkey=https://search.ebscohost.com/login.aspx?direct=true&scope=site&db=nlebk&db=nlabk&AN')
                # print('New URL:' + new_url)
                
                port_json['linking_details'].update({'url': new_url})
                port_json['linking_details'].update({'url_type': {'value': 'static', 'desc': None}})
                port_json['linking_details'].update({'url_type_override': {'value': 'static', 'desc': None}})
